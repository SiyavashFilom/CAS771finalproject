#Siyavash Filom - CAS771 Final Project
#Co-teaching with Three networks
...
############################
# Loading Data
from loader_for_CIFAR import test_dataset, train_dataset, noisy_labels, clean_labels
...
parser.add_argument('--forget_rate', type = float, help = 'forget rate', default = 0.3)
...
num_classes = 10
#################
#Connect loaded data to the model
######################
...
def train(train_loader, epoch, model1, optimizer1, model2, optimizer2,model3, optimizer3):
    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        loss_1, loss_2, loss_3, pure_ratio_1, pure_ratio_2,pure_ratio_3 = loss_coteaching(logits1, logits2, logits3, labels, rate_schedule[epoch],
                                                                     ind, noise_or_not)
        ...
        if (i + 1) % args.print_freq == 0:
            print(
                ...
                % (epoch + 1, args.n_epoch, i + 1, len(train_dataset) // batch_size, prec1, prec2, prec3,loss_1.data[0],
                   loss_2.data[0],loss_3.data[0], np.sum(pure_ratio_1_list) / len(pure_ratio_1_list),
...
def main():
    ...
    print(
        ...
        epoch + 1, args.n_epoch, len(test_dataset), test_acc1, test_acc2, test_acc3, mean_pure_ratio1, mean_pure_ratio2, mean_pure_ratio3))
    # save results
    with open(txtfile, "a") as myfile:
        myfile.write(
    ...
    for epoch in range(1, args.n_epoch):
        ...
        train_acc1, train_acc2, train_acc3, pure_ratio_1_list, pure_ratio_2_list, pure_ratio_3_list = train(train_loader, epoch, cnn1, optimizer1,
                                                                             cnn2, optimizer2)
        # evaluate models
        test_acc1, test_acc2, test_acc3 = evaluate(test_loader, cnn1, cnn2,cnn3)
        # save results
        ...
        print(
            'Epoch [%d/%d] Test Accuracy on the %s test images: Model1 %.4f %% Model2 %.4f %%, Model3 %.4f %%,Pure Ratio 1 %.4f %%, Pure Ratio 2 %.4f %%, Pure Ratio 3 %.4f %%' % (
            epoch + 1, args.n_epoch, len(test_dataset), test_acc1, test_acc2,test_acc3, mean_pure_ratio1, mean_pure_ratio2, mean_pure_ratio3))
        with open(txtfile, "a") as myfile:
            myfile.write(
                str(int(epoch)) + ': ' + str(train_acc1) + ' ' + str(train_acc2) + ' ' + str(test_acc1) + " " + str(
                    test_acc2) +  ' ' + str(mean_pure_ratio2)' ' + str(train_acc3) + ' ' + str(train_acc3) + ' '+ str(mean_pure_ratio3) "\n")
